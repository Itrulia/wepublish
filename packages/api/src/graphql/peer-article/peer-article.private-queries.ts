...
export const getAdminPeerArticles = async (
  ...
  const cursors: Record<string, string> = JSON.parse(stringifiedCursors)
  ...
  const articles = await Promise.all(
    peers.map(peer => {
      try {
        if (cursors && cursors[peer.id] == null) return null

        return delegateToPeerSchema(peer.id, true, context, {
          info,
          fieldName: 'articles',
          args: {cursor: cursors ? cursors[peer.id] : undefined},
          transforms: [
            new ExtractField({
